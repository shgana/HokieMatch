#!/usr/bin/env ts-node
/**
 * ETL script to ingest grade distribution CSV (~33k rows) into Supabase/Postgres.
 * Assumes columns: course_id,instructor,term,section,a_count,b_count,c_count,d_count,f_count,w_count,avg_gpa
 */
...
async function insertGrades(rows: CsvRow[]) {
  const chunkSize = 500

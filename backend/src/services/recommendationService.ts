...
import type { Course, RecommendationResult } from "../types/index.ts"
import { computePassRate, scoreCourse, average } from "../utils/scoring.ts"
...
export async function getEasyCourses(opts: RecommendationOpts = {}): Promise<RecommendationResult[]> {
  ...
  const { data: courses, error } = await db.from(tables.courses).select("*").limit(300)
...
async function getCourseGpa(courseId: string) {
  const { data, error } = await db.from(tables.grades).select("*").eq("course_id", courseId).limit(50)
  if (error) throw error
  return data ?? []
}
